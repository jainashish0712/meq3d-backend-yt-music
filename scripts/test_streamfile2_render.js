  ...
  const p2 = new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      const req = http.get(url, (res) => {
        ...
        // We enforce a 3-second assertion here to tolerate the initial extraction overhead on cloud instances.
        assert.ok(!isNaN(ytDlpTime) && ytDlpTime <= 3000, `yt-dlp task took ${ytDlpTime}ms, exceeding Render limit`);
        ...
        res.on('end', () => {
          ...
          assert.ok(latency <= 4000, `Response time of ${latency}ms exceeded Render 4000ms limit`);
